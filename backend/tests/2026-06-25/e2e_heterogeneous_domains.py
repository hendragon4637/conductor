#!/usr/bin/env python3
"""File 05 — Heterogeneous E2E Test Suite (Varied Product Types).

Stress-tests the full pipeline across 9 scenarios (H1-H9) covering:
- Runnable domains (software_app, cli_script, api_service, data_pipeline)
- Non-runnable domain (research_report)
- Vague input (convention injection)
- Unknown domain (clarify/defer)
- Combined domains (merged profiles)
- Staffing mismatch (gate + reassign/generate)

Usage:
    # Unit tests only (no backend needed):
    uv run python backend/tests/2026-06-25/e2e_heterogeneous_domains.py --unit

    # Full E2E (backend must be running on :8090):
    uv run python backend/tests/2026-06-25/e2e_heterogeneous_domains.py

NOTE: Domain profiles must be seeded first:
    uv run python scripts/seed_domain_profiles.py
"""
...
REQUEST_TIMEOUT = 300  # seconds per API call (LLM calls can be slow)
...
# ── Unit-level tests (no backend required) ──────────────────────────────
...
# ── Integration tests (require backend on :8090) ─────────────────────────
...
def test_h1_finance_tracker_convention_injection():
    """H1: Finance tracker — convention injection should fill FE gap."""
    log("=" * 50)
    log("H1: Finance tracker — convention injection")
    start = time.time()
    result = api("POST", "/api/plans", {
    ...
    check("H1: plan created", bool(plan_id))

    spec = result.get("spec", "")
    conventions_injected = "end-to-end" in spec.lower() or "FE" in spec or "frontend" in spec.lower()
    check("H1: FE convention injected into spec", conventions_injected)

    domain = result.get("domain", "")
    check("H1: domain inferred", domain == "software_app" or not domain)
    log("H1: done (%.1fs)" % (time.time() - start))


def test_h2_vague_budgeting_app():
    """H2: Vague input — should trigger clarification or inject conventions."""
    log("=" * 50)
    log("H2: Vague 'build a budgeting app'")
    start = time.time()
    result = api("POST", "/api/plans", {
        "use_meta_planner": True,
        "goal": "Build a budgeting app",
        "spec": "",
        "quality_intent": "",
    })
    status = result.get("plan_status", "draft")
    check("H2: plan status is clarifying or drafted",
          status in ("awaiting_clarification", "draft"))
    if status == "draft":
        spec = result.get("spec", "")
        check("H2: conventions injected into spec", len(spec) > 20)
    log("H2: done (%.1fs)" % (time.time() - start))


def test_h4_api_only_no_false_fe():
    """H4: API service — NO false FE node."""
    log("=" * 50)
    log("H4: REST API for bookmarks (no UI)")
    start = time.time()
    result = api("POST", "/api/plans", {
        "use_meta_planner": True,
        "goal": "REST API for a bookmarks service with CRUD endpoints",
        "spec": "API only, no UI needed",
        "quality_intent": "RESTful, validated inputs, correct status codes",
    })
    plan_id = result.get("plan_id")
    check("H4: plan created", bool(plan_id))
    spec = result.get("spec", "")
    false_fe = "frontend" in spec.lower() or "html" in spec.lower()
    check("H4: no false FE conventions injected (API-only)", not false_fe)
    log("H4: done (%.1fs)" % (time.time() - start))
...
def test_h6_non_runnable_domain():
    ...
    result = api("POST", "/api/plans", {
        "use_meta_planner": True,
        "goal": "Research report: compare 3 web frameworks on performance and developer experience",
        "spec": "Compare React, Vue, Angular",
        "quality_intent": "Accurate, well-cited, clear conclusion",
    })
    plan_id = result.get("plan_id")
    check("H6: plan created", bool(plan_id))
    nodes = result.get("nodes", [])
    has_runnable_check = any(
        "runnable" in (c.get("criterion", "") or "").lower()
        for n in nodes for c in (n.get("checks") or [])
    )
...
def test_h7_combined_domains():
    ...
    result = api("POST", "/api/plans", {
        "use_meta_planner": True,
        "goal": "Build a habit tracker app AND write its design document",
        "spec": "Fullstack CRUD app + design rationale document",
        "quality_intent": "App works correctly, document is thorough and cited",
    })
    plan_id = result.get("plan_id")
    check("H7: plan created", bool(plan_id))

    status = result.get("plan_status", "draft")
    nodes = result.get("nodes", [])
    if status == "awaiting_clarification":
        # Combined domains triggered clarification — acceptable
        check("H7: clarification questions generated", len(result.get("clarify_questions", [])) > 0)
    else:
        # Plan was formulated with nodes
        check("H7: has nodes", len(nodes) >= 1)
        if nodes:
            task_texts = [n.get("task", {}).get("text", "") for n in nodes]
            has_app_node = any("app" in t.lower() or "tracker" in t.lower() for t in task_texts)
            has_doc_node = any("doc" in t.lower() or "design" in t.lower() for t in task_texts)
            check("H7: has app or doc node", has_app_node or has_doc_node)
    log("H7: done (%.1fs)" % (time.time() - start))
...
def test_h8_unknown_domain_clarify():
    ...
    check("H8: plan created (any status)", bool(result.get("plan_id")))
    log("H8: done (%.1fs)" % (time.time() - start))
...
def run_integration_tests():
    log("Starting integration tests (backend at %s)" % API)
...
def main():
    ...
    if run_unit:
        print("Running UNIT tests only (no backend needed)")
        run_unit_tests()
    else:
        available = check_api_available()
        if not available:
            print(f"Backend not reachable at {API} — running unit tests only")
            run_unit_tests()
        else:
            print(f"Backend reachable at {API} — running full suite")
            run_unit_tests()
