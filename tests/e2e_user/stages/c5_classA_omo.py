"""C5 — Session appears for Plan C (opencode_omo) and completes.

Skips if cap_plan_C not in PLANS_SELECTION.
"""
...
def run(page: Page) -> bool:
    ...
    score_btn = row.locator("[data-testid='session-score-link']")
    assert score_btn.is_visible(), "Expected 'view score' button for done session"
